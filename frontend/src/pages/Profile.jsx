...
const Profile = () => {
  ...
  const formatMoney = (value) => {
    ...
    return new Intl.NumberFormat(i18n.resolvedLanguage === "bg" ? "bg-BG" : "en-US", {
  ...
  const quickLinks = [
    {
      to: "/building",
      label: t("profile.quickLinks.buildingLabel"),
      desc: t("profile.quickLinks.buildingDesc"),
    },
    {
      to: "/entrance",
      label: t("profile.quickLinks.entranceLabel"),
      desc: t("profile.quickLinks.entranceDesc"),
    },
    {
      to: "/apartment",
      label: t("profile.quickLinks.apartmentLabel"),
    ...
    {
      to: "/offer",
      label: t("profile.quickLinks.offerLabel"),
      desc: t("profile.quickLinks.offerDesc"),
    },
  ];
  ...
  if (loading) {
    return (
      <main className="profile-layout">
        <p className="profile-state">{t("profile.loading")}</p>
      </main>
    );
  }
  ...
  if (error) {
    return (
      <main className="profile-layout">
        <p className="profile-state profile-state--error">{error}</p>
      </main>
    );
  }
  ...
  if (!user) {
    return (
      <main className="profile-layout">
        <p className="profile-state">{t("profile.loadingData")}</p>
      </main>
    );
  }
    ...
    <main className="profile-layout">
      <section className="profile-hero">
        <div className="profile-hero-left">
          <img src="/images/profile.png" alt="Профил" className="profile-avatar" />
          ...
          <div className="profile-hero-text">
            ...
            <div className="profile-tags">
              <span className="tag">
                {t("profile.tagBuilding")}
                {user.building || "—"}
              </span>
              <span className="tag">
                {t("profile.tagEntrance")}
                {user.entrance || "—"}
              </span>
              <span className="tag">
                {t("profile.tagApartment")}
                {user.apartment || "—"}
              </span>
              <span className="tag">
                {t("profile.tagClientNumber")}
                {user.clientNumber || "—"}
              </span>
            </div>
          </div>
          ...
          <div className="card card-stats">
            ...
            <div className="stats-grid">
              <div className="stat-item">
                ...
                <span className="stat-value">{formatMoney(user.balance)} лв.</span>
              </div>
                  ...
                  <div className="finance-row" key={index}>
                    ...
                    <span className={`status ${payment.status}`}>
                      {payment.status === "paid"
                        ...
                        : t("profile.statusOverdue")}
                    </span>
                  </div>
                ))
              ) : (
                <div className="finance-empty">{t("profile.noPayments")}</div>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="card card-events">
            ...
            <ul className="events-list">
              {events.length > 0 ? (
                ...
                <li className="events-empty">{t("profile.noData")}</li>
              )}
            </ul>
          </div>
        </div>
